...
st.image("banner.png")
st.markdown("""
            This application is designed to streamline the process of generating G-code for controlling your Ender 3 Pro 3D printer to mix iron perchloride for PCB etching. By utilizing the heated bed and (x,y) plate movements of the Ender 3, this app ensures efficient mixing of the fluid, significantly speeding up the PCB manufacturing process.
...
with st.form("my_form"):
    ...
    bedTemp_input = st.number_input(
        "Bed temperature (0°C if no heat needed)", value=30)
    ...
    if submitted := st.form_submit_button("Generate the G-code"):
        gcode_content = generate_gcode(file_name_input, bedTemp_input, timeMix_input, numWait_input,
                                       speed_input, zPosition_input, frontPosition_input, backPosition_input)

if gcode_content:
    st.download_button(label="Download G-code", data=gcode_content,
                       file_name=file_name_input, mime="text/plain")
